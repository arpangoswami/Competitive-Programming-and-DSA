...
using namespace std;
int recMinSum(int idx, int n, vector<int> &arr, int s1, int s2, int **dp) {
	if (dp[idx][s1] != -1) {
		return dp[idx][s1];
	}
	if (dp[idx][s2] != -1) {
	...
	if (idx == n) {
		...
		int ans1 = recMinSum(idx + 1, n, arr, s1 + arr[idx], s2, dp);
		int ans2 = recMinSum(idx + 1, n, arr, s1, s2 + arr[idx], dp);
		ans = min(ans1, ans2);
	}
	dp[idx][s1] = ans;
	dp[idx][s2] = ans;
...
int32_t main(int32_t argc,char const *argv[]) {
	...
	int minSum = recMinSum(0, n, arr, 0, 0, dp);
	int ans = (s + minSum) / 2;
	cout << ans;
	for (int i = 0; i < n; i++) {
		delete []dp[i];

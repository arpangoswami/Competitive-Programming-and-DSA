...
void printRightView(treeNode *root,int lvl,int &max_lvl){
	if(!root){
		return;
	}
	if(lvl > max_lvl){
	...
	printRightView(root->right,lvl+1,max_lvl);
	printRightView(root->left,lvl+1,max_lvl);
}

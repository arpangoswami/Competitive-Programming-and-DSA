...
const int len = 10;
bool isSafe(int x,int y,int m,int n,char c,int idx,vector<vector<bool>> &visited) {
	return ((x>=0) && (y>=0) && (x<m) && (y<n) && c == path[idx] && !visited[x][y]);
}
bool pathExist(int idx, int x, int y, int m, int n, vector<string> &board, vector<vector<bool>> &visited) {
	...
	for (int i = 0; i < 8 ; i++){
		...
		if(isSafe(nx,ny,m,n,board[nx][ny],idx,visited)){

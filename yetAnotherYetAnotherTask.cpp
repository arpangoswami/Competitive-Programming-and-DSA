...
using namespace std;
int kadSum(vector<int> &arr,int j){
	//j is the current maximum in the segment
	for(int &x:arr){
		if(x > j){
			x = -1e9;
		}
	}
	//all elements bigger than j is minus infinity taking them into account will be wasteful
	int ans = 0,curr = 0;
	for(int x:arr){
		curr += x;
		ans = max(ans,curr);
...
void solveQuestion(){
	...
	for(int j = 30;j >= 1;j--){
		vector<int> temp = arr;
		int curr = kadSum(temp,j);
		//curr - j as bob takes one card having the highest value.
		ans = max(ans,curr - j);
	}
	cout<<ans<<'\n';
}
